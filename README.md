...
| `BLOCKBOT_REMOTELIST` | `ON`, `OFF`, URL, or empty | This variavke indicates if we should get a list of blocked User Agent (sub)strings from a remote URL. If set to yes/on/enabled, it will use the list available at <https://github.com/sdr-enthusiasts/docker-reversewebproxy/blob/main/blockbot-ua-list>. If set to a URL, it will use the list at that URL (make sure you link to the "raw" file!). If omitted, empty, or set to no/off/disabled, it won't use a remote list |
